...
- Normalizes generated diagrams with a human-style compact tree layout by default, with Graphviz support for general directed graphs.
...
`apply` uses auto layout by default. It uses `compact-tree` for system/module structure charts, can use Graphviz `dot` for general directed graphs, and writes the result as an editable VML group rather than a raster image. Use `--layout manual` only when the JSON coordinates are already carefully designed.
...
```json
{
  ...
  "layout": { "mode": "auto", "engine": "compact-tree" },
...
The bundled script uses direct DOCX VML group markup so it can work headlessly. LibreOffice and Microsoft Word remain useful for visual review and final fidelity checks.
...
- 默认使用更像人工绘制的紧凑树形布局，普通有向图可使用 Graphviz 布局，避免节点歪斜、连线混乱和文字溢出。
- 将图写回为可编辑的 Word 绘图形状。
...
`apply` 默认启用自动布局。系统功能结构图默认使用 `compact-tree`，普通有向图可用本机 Graphviz `dot` 辅助布局，最后写入可编辑的 Word VML 形状组。只有坐标已经精心设计时才使用 `--layout manual`。
